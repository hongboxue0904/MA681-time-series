...
plot(yr,lake, type="o",pch=20,main = "Lake Huron Water Level" , ylab="feet" )
acf=acf(lake)
...
lake1<-lake1[1:length(lake2)]


level <- (.8364*lake1)+ (.1857*lake2)+(-.0174)
yr1 <- yr[1:length(lake2)]
yr2 <- yr[-1]
yr2 <- yr2[1:length(lake2)]
plot(yr,lake, type="l",pch=20,main = "Lake Huron Water Level" , ylab="feet" )
lines(yr2,level, col="red")
modelresid = lake2 - level
...
level2 <- (1.0534*lake1)+ (-.2668*lake2) 
